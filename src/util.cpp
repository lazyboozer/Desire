...
const char * const BITCOIN_CONF_FILENAME = "desire.conf";
const char * const BITCOIN_PID_FILENAME = "desired.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "desire" is a composite category enabling all Desire-related debug output
            if(ptrCategory->count(string("desire"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DesireCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DesireCore
    // Mac: ~/Library/Application Support/DesireCore
    // Unix: ~/.desirecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DesireCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DesireCore";
#else
    // Unix
    return pathRet / ".desirecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty desire.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override desire.conf
